...
| `{{FEATURE_SPECS_DIR}}` | arguments.yml（`/athena-discovery` bind 後展開） | Discovery accepted rule / behavior truth 根 | `specs/backend/packages/01-計費/features` |
...
8. **Migration**：啟動時 Flyway 自動套用 `src/main/resources/db/migration/V*__*.sql`（starter 預設無 migration 檔，本步驟在 `/athena-discovery` → schema-analysis 後才有資料可驗證）。
