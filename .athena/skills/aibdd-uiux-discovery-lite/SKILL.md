...
Happy-path 視覺探索規劃器｜從 `/athena-discovery` 產物推導 **feature-mentioned screens only** 的最小 Pencil brief。它保留下游需要的三個合約 section（`COMPONENT CATALOG` / `FRAME COMPOSITION TABLE` / `ANCHOR NAME TABLE`），但把 component state 收斂成 primary/canonical 狀態，避免完整 `/aibdd-uiux-discovery` 的 component × state 展開造成畫面爆炸。
...
2. 確認 discovery 產物存在，否則停止。
   ...
   2.5 IF assertion fails:
       2.5.1 EMIT "discovery 產物缺失；請先跑 /athena-discovery" to user
       2.5.2 STOP
...
- `/athena-discovery` — 上游；產出 spec.md / activities / features / atomic-rules 為本 skill 的 INPUT SSOT
