...
2. BRANCH `replay_from` 決定從哪一步起跑：
   1. `athena-discovery` → 從步驟 3 開始（discovery → plan → spec-by-example 全跑）。
...
3. DELEGATE `/athena-discovery`，payload 依 `.athena/skills/aibdd-reconcile/references/planner-handoff-contract.md` 的 Discovery Payload 組裝。
   1. 若回報 status 不是 completed，STOP 並回報用戶：reconcile 停在 /athena-discovery，下游未執行。

4. TRIGGER session 進度推進 script，把指標推到 `aibdd-plan`：

   ```bash
   python3 ${CLAUDE_PLUGIN_ROOT}/.athena/skills/aibdd-reconcile/scripts/python/manage_reconcile_session.py advance \
     "<active_session_path>" "aibdd-plan"
   ```

5. DELEGATE `/aibdd-plan`，payload 依 `.athena/skills/aibdd-reconcile/references/planner-handoff-contract.md` 的 Plan Payload 組裝。
...
7. DELEGATE `/aibdd-spec-by-example-analyze`，payload 依 `.athena/skills/aibdd-reconcile/references/planner-handoff-contract.md` 的 Spec-by-Example Payload 組裝。
