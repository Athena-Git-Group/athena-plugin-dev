...
description: >
  ...
  然後委派 /athena-carry-on-engineering-plan 以 feedback loop 審查機制逐步執行。
  當使用者說「specformula」「新需求」「走完整開發流程」「前後端開發」，
  或想對一個需求走 BDD 全流程時觸發。
  前提：arguments.yml 已存在（/athena-kickoff 已完成）。
---

# 角色

需求工程計畫產生器。你將任意需求轉化為結構化工程計畫，產出 plan.md + Phase 卡片，
然後委派 `/athena-carry-on-engineering-plan` 以 human-in-the-loop 審查機制逐步執行。
...
```bash
uv run .claude/skills/athena-specformula/scripts/generate-plan.py \
...
- **DELEGATE `/athena-carry-on-engineering-plan`**，傳入 `${PLAN_DIR}` 路徑
...
|-------|------|-----------|-----------|---------|
| 01 | Requirement Analysis | `assets/01-requirement-analysis.md` | `/athena-discovery` | Execution Plan + Activity Diagrams + Feature Rules |
| 02 | Entity Modeling | `assets/02-entity.md` | `/athena-form-entity-spec` | erm.dbml |
| 03 | BDD Analysis | `assets/03-bdd-analysis.md` | `/athena-form-bdd-analysis` | 系統抽象 + 句型模型 + Feature Examples |
| 04 | API Contract | `assets/04-api-contract.md` | `/athena-form-api-spec` | api.yml |
...
**Reconciler 架構** — Phase 02-04 的 form skill 均以 desired-state reconciliation 運作（derive desired → read current → compute diff → preview → apply）。共用合約見 `athena-core/references/reconciler-contract.md`。

## Implementation（實作）

| Phase | 名稱 | Asset 卡片 | 觸發 Skill | 核心產出 |
|-------|------|-----------|-----------|---------|
| 05 | Backend TDD | `assets/05-backend-tdd.md` | `/athena-auto-control-flow` | Step Defs + Models + Endpoints + Migrations |
