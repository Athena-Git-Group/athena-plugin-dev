...
觸發 skill：`/athena-auto-control-flow`（內部自動從 arguments.yml 路由語言變體）
...
**DELEGATE `/athena-auto-control-flow`**，對每個 new feature 依序走：

| Phase | Skill | 做什麼 |
|-------|-------|--------|
| Red | `/athena-auto-red` | Schema Analysis → Step Template → 寫 E2E 測試（欄位名 = api.yml） |
| Green | `/athena-auto-green` | 實作至測試通過 |
| Refactor | `/athena-auto-refactor` | 程式碼品質提升 |
