...
Preferred invocation:
    ...
    uv run .athena/skills/aibdd-core/scripts/run_dsl_cli.py <subcommand> ...

Legacy package entry (requires PYTHONPATH and pre-installed runtime deps):

    PYTHONPATH=.athena/skills/aibdd-core/scripts python3 -m dsl_cli <subcommand> ...

`--boundaries-root` defaults to the canonical on-disk location
(.athena/skills/aibdd-core/assets/boundaries/) so production callers pass only
