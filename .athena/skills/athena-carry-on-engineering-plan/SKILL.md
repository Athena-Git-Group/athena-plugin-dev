---
name: athena-carry-on-engineering-plan
description: >
...
## Consistency Check（委派 `/athena-consistency-analyzer`）
...
2. 觸發 `/athena-consistency-analyzer`，傳入這些檔案路徑。
3. `/athena-consistency-analyzer` 會自行完成掃描、展示 issue 清單、互動迴圈（+A/+S/+X）、以及寫入 `{plan_dir}/clarify-log.md`。
