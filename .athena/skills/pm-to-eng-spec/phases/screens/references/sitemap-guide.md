...
- **design/ 存在** → 畫面清單、版面、可見元素、狀態以視覺稿為準；clarified.md 提供行為 / 規則 / 權限。兩者**對照**：
  - 視覺稿有、clarified.md 沒提到的畫面 / 元素 → 標 `待釐清:設計稿有 X，需求未提，是否納入？` 回報，**不擅自當定論**。
  - clarified.md 要求、視覺稿沒畫的（如某錯誤態）→ 標 `待補設計:X 狀態無對應稿`，仍寫進 screen-map（行為需求優先）。
- **design/ 不存在** → 純從 clarified.md 的使用情境推導畫面（仍照本檔格式）。

> 視覺稿是「長什麼樣」的真相，clarified.md 是「該怎麼行為」的真相。screen-map 要同時忠於兩者。
...
- ❌ 自行新增 clarified.md 沒提的畫面（違反非協商規則 #1）。
