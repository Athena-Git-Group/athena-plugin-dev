# API intent DSL 格式錨點（Format Anchor）— api stage 精簡版

> 本 stage 的 **API intent DSL** 格式定義。**裁剪範圍**：
...
> auth / 分頁（`access` / `exposes.list`）待本 stage 納入範圍時再擴充本錨點與 transpiler。
