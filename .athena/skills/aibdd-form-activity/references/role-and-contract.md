...
Formulation skill。綁定 DSL = `.activity`（扁平事件流）。被 `athena-discovery` DELEGATE；只把上游 `activity_analysis.activity` 中的 modeling elements 翻譯成 artifact，不自行重新分析需求、不檢查顆粒度、不新增 Actor / Action / Decision / Fork。
...
- `self-contained`、phase-slice merge、feature-index anti-pattern、以及 branch terminal 完整性屬於上游 `/athena-discovery` 的建模責任。
