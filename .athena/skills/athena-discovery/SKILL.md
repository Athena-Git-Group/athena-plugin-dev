---
name: athena-discovery
description: >
  AIxBDD 需求探索主入口（取代 athena-discovery 成為 Phase 01）。三段式 sub-SOP：
  01-sourcing-and-packaging 鎖定 plan package 與 function package 顆粒度；
  02-feature-file-list-analyze 推導 feature file 清單；03-atomic-rules-analyze
  萃取 atomic rules。產出 discovery-sourcing.md、PLAN_SPEC、impact-matrix.yml
  與 function-package feature 骨架。前置：`/aibdd-kickoff` 已產出 arguments.yml。
  下游：`/aibdd-plan` → `/aibdd-tasks` → `/aibdd-{red,green,refactor}-execute`。
  觸發詞：athena-discovery、AIxBDD 需求探索、aibdd 需求分析。
user-invocable: true
---
...
4. 和用戶說道（可使用不同詞彙但維持語意）：「OK 基本上 /athena-discovery 所有分析步驟都完成了，如沒問題，可以執行 /aibdd-plan，來正式進入內外部邊界介面的規劃以及 DSL 的設計。」
