...
Plugin 提供以下原生 slash commands（短形式由 harness 路由到對應 skill）：
...
若有命名衝突，可使用 fully-qualified 形式 `/athena-dev-plugin:athena-flow` 等。
Commands 是 thin wrapper，邏輯仍在 `skills/<name>/SKILL.md`——團隊要修改行為改 skill 即可。

## 強制規則

安裝本 plugin 後，**任何程式碼變更都必須先跑 `/athena-point`**。
這條規則由 plugin 內建的 `PreToolUse` hook（`hooks/require-point.sh`）執行：
找不到 `points/*.md` 時，hook 會 block `Edit` / `Write` / `MultiEdit` / `NotebookEdit` tool。

### Hook 觸發範圍

Hook 只在 cwd 看起來是 Athena 專案（有 `.athena/` 或 `points/` 目錄）時生效，
不會干擾未啟用 Athena 流程的其他專案。

### 自我保護路徑（永遠放行）

為了讓使用者在 hook 出狀況時仍能修復，以下路徑不受 gate 限制：

- `hooks/**`（gate 機制本身）
- `.claude-plugin/**`（plugin manifest）
- `commands/**`（slash command 入口）
- `.claude/**`（harness settings）
- `.athena/**`（團隊設定、escape marker）
- `points/**`、`handoffs/**`（評分與交接 artifact）

### Escape Hatch

若需要在特定情境繞過 gate（hotfix、純文件改動、其他工具自動產生的編輯）：
