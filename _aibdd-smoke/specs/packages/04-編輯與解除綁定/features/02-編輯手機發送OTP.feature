...
@ignore
Feature: 為已綁定手機之編輯流程發送 SMS OTP 驗證碼
    # Action：已登入會員在帳號設定點擊手機旁「編輯」並輸入新手機後，點擊「發送驗證碼」由後端發送 SMS OTP。
    # 來源：PRD §5.7、BR-45/46、T-91、T-93。
    ...
    Rule: 後置（狀態） - 後端應發送 SMS OTP 至新手機號碼
    ...
    Rule: 後置（回應） - 新手機已被使用應拒絕並提示「此手機號碼已被使用，請改用其他手機號碼」
        - 後端錯誤訊息由前端直接顯示（BR-33、T-93）。
