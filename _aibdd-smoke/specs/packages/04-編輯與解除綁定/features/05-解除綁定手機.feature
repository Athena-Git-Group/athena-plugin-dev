...
@ignore
Feature: 已登入會員解除綁定手機
    # Action：已雙綁定會員在帳號設定點擊手機旁「解除綁定」，通過二次確認彈窗後解除（不需密碼，v1.7）。
    # 來源：PRD §5.8、BR-47/48/49/50、T-99、T-100、T-103、T-105。
    ...
    Rule: 後置（狀態） - 系統應將該會員對應 Customer 的 mobilePhone 欄位清空
        - 該手機號碼不可再用於登入（BR-49、T-103）。
    ...
    Rule: 後置（狀態） - 解除後該手機號碼應被釋放可被其他使用者註冊或綁定
        - 釋放策略（BR-50）。
        Example: <操作> 後 <狀態主詞> 變為 <新狀態>
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
    ...
    Rule: 後置（狀態） - 解除後該會員仍可使用 Email 登入
        - 另一種聯絡方式不受影響（BR-49）。
        Example: <操作> 後 <狀態主詞> 變為 <新狀態>
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
    ...
    Rule: 後置（回應） - 解除成功應在帳號設定頁將手機改為未綁定狀態並顯示「綁定」按鈕
        - 切換至未綁定狀態 UI（T-99）。
