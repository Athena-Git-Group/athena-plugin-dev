...
@ignore
Feature: 在 App 端啟用生物辨識登入
    # Action：已登入會員在帳號設定點擊「啟用生物辨識登入」，通過裝置原生生物辨識驗證後，向後端申請並儲存 refresh token。
    # 來源：PRD §5.10、BR-57/58/59、T-117、T-118、T-124。

    Rule: 前置（參數） - 使用者必須處於 App 端
        - Web 不支援生物辨識（BR-57、T-124）。
    ...
    Rule: 前置（狀態） - 啟用時使用者必須通過一次裝置原生生物辨識驗證
        - 確認本人操作（BR-58）。
        - 失敗或取消則不啟用（T-118）。
        Example: <主詞> 不滿足 <條件> 時 <操作> 失敗
          # @dsl
    ...
    Rule: 後置（狀態） - 後端應為該會員裝置核發 refresh token
        - 多裝置可分別啟用（T-125）。
    ...
    Rule: 後置（狀態） - App 應將 refresh token 加密儲存於裝置原生安全區域
        - iOS Keychain / Android Keystore；不上傳雲端（BR-59）。
        Example: <操作> 後 <狀態主詞> 變為 <新狀態>
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
          # rule: 先讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/shared-given-law.md 理解 shared arrangement 作為你的推理流程，再讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/given-delta-postcondition-state.md 補 `後置（狀態）` 所需建構出的可量測初始狀態。
    ...
    Rule: 後置（回應） - 啟用成功應在帳號設定顯示「生物辨識登入：已啟用」
        - 提供後續停用入口（T-117）。
