...
@ignore
Feature: 為手機忘記密碼流程發送 SMS OTP 驗證碼
    # Action：使用者在忘記密碼頁輸入手機，點擊「發送驗證碼」由 forgot/mobile 觸發 SMS OTP 發送。
    # 來源：PRD §5.4 步驟 1-4、BR-21/23/27/29/36/37、T-46、T-49。

    Rule: 前置（參數） - 使用者必須提供格式合法的台灣手機號碼
        - 前端識別為手機後正規化為 `+886` + 9 位數字（BR-27、BR-29）。
    ...
    Rule: 前置（狀態） - 該手機號碼必須對應一個未刪除的會員中心帳號
        - 未註冊應回傳「此手機號碼尚未註冊」（T-49）。
    ...
    Rule: 後置（回應） - 同一號碼當日 OTP 發送達上限時應拒絕並提示「今日驗證碼發送次數已達上限，請明日再試」
        - 每日發送上限由 `wrs_otp_daily_limit` 控制（BR-23）。
