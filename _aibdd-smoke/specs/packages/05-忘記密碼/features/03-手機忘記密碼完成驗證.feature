...
@ignore
Feature: 以 SMS OTP 完成手機忘記密碼身份驗證
    # Action：使用者輸入收到的 OTP，呼叫 verify-otp（FORGOT_PASSWORD 用途）；驗證成功後核發 otpToken 進入重設密碼步。
    # 來源：PRD §5.4 步驟 5-6、BR-21/22/52、T-46、T-47、T-48、T-58。
    ...
    Rule: 前置（狀態） - 該 OTP 必須仍在有效期限內
        - 有效期由 `wrs_otp_expiry_sec` 控制（BR-22、T-48）。
        - 過期應提示「驗證碼已過期」。
    ...
    Rule: 前置（狀態） - 該 OTP 必須是 FORGOT_PASSWORD 用途且尚未被使用過
        - REGISTER / LOGIN 用途 OTP 不互通（BR-52、T-58）。
        Example: <主詞> 不滿足 <條件> 時 <操作> 失敗
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
          # rule: 先讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/shared-given-law.md 理解 shared arrangement 作為你的推理流程，再讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/given-delta-precondition-state.md 補 `前置（狀態）` 所需建構出的系統合法狀態。
    ...
    Rule: 後置（狀態） - 驗證成功時系統應核發限時 otpToken 供後續 forgot/reset 使用
        - otpToken 有效期由 `wrs_otp_token_expiry_sec` 控制（建議 600 秒）。
    ...
    Rule: 後置（回應） - 驗證成功應回傳成功結果讓使用者進入重設密碼畫面
        - 前端據此導向新密碼輸入步驟（T-46）。
