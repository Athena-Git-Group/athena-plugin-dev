...
@ignore
Feature: 以 otpToken 完成手機忘記密碼之重設密碼
    # Action：使用者於重設密碼畫面輸入新密碼，呼叫 forgot/reset API；後端驗 otpToken 後更新密碼。
    # 來源：PRD §5.4 步驟 6-7、BR-04/21、T-46、T-50、T-51、T-74。

    Rule: 前置（參數） - 使用者必須提供前一步取得且未過期的 otpToken
        - otpToken 僅限一次使用（T-74）。
    ...
    Rule: 前置（參數） - 使用者必須提供長度至少 8 字元的新密碼
        - 沿用既有密碼規則（BR-04）。
        Scenario Outline: <參數名> = <無效值> 時 <操作> 失敗
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
    ...
    Rule: 前置（狀態） - 對應 otpToken 之手機號碼必須仍對應一個未刪除的會員中心帳號
        - 防止 token 偽造或過期使用（T-73）。
    ...
    Rule: 後置（回應） - 重設成功應導向登入頁並提示使用者以新密碼登入
        - 前端據此導向登入後狀態（T-46）。
