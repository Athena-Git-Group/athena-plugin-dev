...
@ignore
Feature: 已登入會員於帳號設定補綁 Email
    # Action：已登入會員在帳號設定點擊「綁定 Email」，輸入 Email 後直接綁定（不需 OTP）。
    # 來源：PRD §5.5、BR-11/12/14/15/26、T-39、T-40、T-42。
    ...
    Rule: 前置（狀態） - 該會員必須尚未綁定 Email
        - 已雙綁定者不再顯示「綁定」按鈕（T-42）。
        - 既有 Email 會員依方向規則僅可補綁手機（BR-11、BR-26）。
    ...
    Rule: 前置（狀態） - 該 Email 必須尚未被其他帳號使用
        - 唯一性檢查（BR-12、T-40）。
        Example: <主詞> 不滿足 <條件> 時 <操作> 失敗
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
    ...
    Rule: 後置（狀態） - 系統應將該 Email 寫入該會員對應 Customer 的 email 欄位
        - 完成後兩種聯絡方式皆可登入（BR-15、T-39）。
    ...
    Rule: 後置（回應） - 綁定成功應更新帳號設定頁顯示 Email 與「編輯」按鈕
        - 切換至已綁定狀態 UI。
    ...
    Rule: 後置（回應） - Email 已被使用應拒絕並提示「此 Email 已被使用，請改用其他 Email」
        - 後端錯誤訊息由前端直接顯示（BR-33、T-40）。
