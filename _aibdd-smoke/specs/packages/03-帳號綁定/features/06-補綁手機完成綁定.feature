...
@ignore
Feature: 以 SMS OTP 完成補綁手機並啟用雙管道登入
    # Action：已登入會員輸入收到的 OTP，verify 成功後將手機寫入該會員的 Customer 紀錄。
    # 來源：PRD §5.5、BR-12/13/15、T-38、T-43。
    ...
    Rule: 後置（狀態） - 系統應將該手機寫入該會員對應 Customer 的 mobilePhone 欄位
        - 完成後兩種聯絡方式皆可登入（BR-15、T-38）。
    ...
    Rule: 後置（回應） - 綁定成功應更新帳號設定頁顯示手機與「編輯」按鈕
        - 切換至已綁定狀態 UI。
