...
@ignore
Feature: 為手機號碼註冊發送 SMS OTP 驗證碼
    # Action：使用者於統一輸入欄位輸入手機 + 密碼後點擊「發送驗證碼」，前端正規化後呼叫 sign-up/mobile 觸發 SMS OTP 發送。
    # 來源：PRD §5.1 步驟 1-4、BR-03/04/06/09/27~31/36/37、T-06、T-14、T-59、T-64、T-78、T-79、T-113、T-114。

    Rule: 前置（參數） - 使用者必須提供格式合法的台灣手機號碼
        - 前端識別為手機後正規化為 `+886` + 9 位數字（BR-03、BR-27、BR-29）。
        - 識別為手機但不符 +886 + 9 開頭 9 碼者必須前端攔截並提示「請輸入有效的台灣手機號碼」（BR-29、T-14、T-78）。
    ...
    Rule: 後置（狀態） - 後端應發送 SMS OTP 至該手機號碼
        - 透過 IT 服務簡訊架構發送（SMS_SEND_INFO_LIST + bacchusTask + twsms.com，BR-37）。
    ...
    Rule: 後置（回應） - 發送成功應回傳 OTP uuid 供後續 verify-otp 使用
        - otpToken 沿用既有 uuid 機制（PRD 附錄 B Q-07）。
    ...
    Rule: 後置（回應） - 重送間隔內再次請求應拒絕並提示倒數秒數
        - OTP 重送最小間隔 60 秒（BR-23）、頻率限制由後端處理（BR-36、T-54、T-113）。
        Example: <操作> 後 <回應主詞> 為 <期望值>
          # @dsl
          # handler-candidate-kinds: state-builder | operation-invoke | time-control | external-stub
          # rule: 先讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/shared-given-law.md 理解 shared arrangement 作為你的推理流程，再讀 ${SKILL_HOME}/aibdd-core/assets/boundaries/web-service/dsl-arrangement-rules/given-delta-postcondition-response.md 補 `後置（回應）` 所需建構出的系統合法狀態。
    ...
    Rule: 後置（回應） - 同一號碼當日 OTP 發送達上限時應拒絕並提示「今日驗證碼發送次數已達上限，請明日再試」
        - 每日發送上限由系統參數 `wrs_otp_daily_limit` 控制（BR-23、T-56、T-114）。
