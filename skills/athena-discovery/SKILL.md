...
此原則貫穿所有階段、所有 Spec Skill。具體規則見 `/athena-composition-analysis`。摘要：
...
Activity 視圖統一由 `/athena-form-activity` 處理。
...
| # | 任務 | 標記 `completed` 的條件 |
|---|------|------------------------|
| 1 | Composition Analysis | `/athena-composition-analysis` DELEGATE 完成、盤點表產出 |
...
2. **DELEGATE** `/athena-composition-analysis`
...
2. 依 Execution Plan 逐一處理：
   - **create**：**DELEGATE** `/athena-form-activity` 新建 Activity Diagram + **DELEGATE** `/athena-form-feature-spec` 新建 Feature Files（Rules only，無 Examples，標記 `@ignore`）
...
**DELEGATE** `/athena-form-feature-spec` 執行 F1-F6 面向覆蓋率清單：
