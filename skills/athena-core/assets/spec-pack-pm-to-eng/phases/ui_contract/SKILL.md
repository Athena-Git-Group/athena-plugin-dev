...
description: >
  ...
  由 pm-to-eng-flow 在 target = frontend / fullstack 時以全新 agent 觸發。前提：specify 已 READY、screens 已產出。
---

# ui_contract · 互動契約（階段 3 / 前端 track）

> 流水線位置：score(gate) → clarify(gate) → specify(gate) → screens → **ui_contract**(前端) → gherkin。
...
- `specs/<slug>/specify/spec.md`（首行 STATUS 必須為 READY）——行為 / 規則 / 格式值真源；
  fixtures 素材取自其「資料維度與範例資料」段
...
- `specs/<slug>/ui_contract/ui-contract.md`
  ...
  - 欄位級驗證與呈現規格：client 端驗證規則（必填 / 格式 / 範圍 / 跨欄位）+ 金額 / 日期 / 數值的顯示與輸入格式（消費 spec.md 的格式決策，不重新決定業務規則）
...
1. **盤點來源** — 讀 `specify/spec.md`、screen-map（＋ `design/` 視覺稿若有）；檢查 `api/openapi.yaml` 是否存在（決定走消費或 API-First 反推）。
...
6. **規劃 API 層四件套** — typed client 函式（對齊 operationId、`$fetch<T>`）/ Zod schema（搬 openapi 約束、型別用 `z.infer`）/ fixtures（取自 spec.md「資料維度與範例資料」段的範例資料）/ MSW handler（覆蓋成功 + 錯誤碼）。**只規劃、不寫程式碼**。（`api-layer-guide.md`）
7. **接錯誤碼** — 互動的失敗分支對應後端 Error schema / 狀態碼，供下游 gherkin 前端場景驗證。（§3）
8. **定 client 驗證與呈現格式** — 為每個輸入欄位列 client 端驗證規則（必填 / 格式 / 範圍 / 跨欄位），標明「送出前擋下」或「僅提示、仍送後端」；每個金額 / 日期 / 數值欄位定顯示格式（千分位 / 小數精度 / 對齊 / 負數 / 幣別）與輸入行為（遮罩 / 貼上 / 即時試算）。格式的**值**（小數位、幣別、時區）以 spec.md / openapi 為準，本步只定「前端怎麼呈現與輸入」，不重定業務規則。
...
- [ ] 每個金額 / 日期 / 數值欄位有顯示格式與輸入行為規格，格式值回指 spec.md / openapi。
...
6. 缺少必要上游 artifact（`specify/spec.md` / screen-map）時，回報並中止。
7. 金額 / 日期 / 數值的**格式值**（小數位、幣別、時區）以 spec.md / openapi 為單一事實來源；ui_contract 只定前端呈現與輸入行為，不自訂業務格式規則。
