...
| 方向 | 內容 |
|------|------|
| Input | .feature skeleton (from /athena-form-activity) &#124; `${ES_SPEC_PATH}` (ES spec) &#124; User idea |
...
來自 `/athena-form-activity` 連動生成的骨架。骨架已有 `@ignore @command` / `@ignore @query` + Feature header + 部分 Rule 框架。
...
**完整格式定義、品質標準與範例見 `../athena-form-activity/references/cic-format.md`。**
