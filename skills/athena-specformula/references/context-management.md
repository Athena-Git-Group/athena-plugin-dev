...
|---------------|---------------|
| Phase 01 | `/athena-discovery`（統一流程：組成分析 → Structural Read → Impact → 行為設計） |
| Phase 02 | `/athena-form-entity-spec` |
| Phase 03 | `/athena-form-bdd-analysis`（內部 lazy load web-backend preset） |
| Phase 04 | `/athena-form-api-spec` |
| Phase 05 | `/athena-auto-control-flow` → 內部呼叫 `/athena-auto-red`、`green`、`refactor`（各自 lazy load variant references） |
| Phase 06 | `/athena-auto-frontend-apifirst-msw-starter` → `msw-api-layer` → `nextjs-pages`（依序載入） |
| Phase 07 | `/athena-frontend-e2e-activity-testplan` |
