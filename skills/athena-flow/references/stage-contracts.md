...
無論哪個 standard stage，handoff artifact 都必須包含 Base 模板的六欄——模板全文、mini-handoff 模板與機械契約見 `agent-handoff.md`。

> **Flow-inline stage 不產出 handoff artifact**，改用 flow context 傳遞資訊。

## Agent 隔離原則

- **Standard stage 由全新 agent 執行，不共享上下文**：skill 在乾淨 agent 中被載入，沒有前一個 stage 的對話歷史，所有前置資訊必須從 handoff artifact 讀取。目的：避免 context 過大、避免前階段推理污染、讓團隊可替換任意 stage 實作。
- **Flow-inline stage 在 flow agent 中內聯執行，共享 flow context**：輕量操作不值得 fresh agent 的成本。
...
2. Skill 的輸出必須符合該 stage 契約的「必要輸出」；Standard stage 產出 handoff artifact 到 `handoffs/`，Flow-inline stage 更新 flow context
3. Handoff artifact 必須包含通用六欄
4. **Standard stage 必須由全新的 agent 執行**——不得讓同一個 agent 處理多個 standard stage（**例外**：Lightweight 路由的 review-ship 可合併為一個 agent）；**Flow-inline stage 在 flow agent 中執行**，不開 fresh agent
5. Skill 不得假設自己能存取前一個 stage 的對話脈絡——一切靠 artifact 或 flow context
6. **只在 gate PASS 後才 commit** — gate FAIL 不觸發任何 git 操作
7. **Git hook 失敗不阻斷流程** — git 操作失敗時記錄警告，stage 流程繼續（除非使用者設定 strict mode）
8. **只有 Ship 可以 push** — pre-build / post-build 僅做 local 操作，push 與 merge 到目標環境由 ship stage 執行
9. **Git 操作冪等** — 分支已存在就切換，commit 無變更就跳過
