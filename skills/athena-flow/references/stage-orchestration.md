...
```
Lightweight:
/point → [pre-build] → /build (single agent) → [/verify] → [post-build] → /review-ship
```
...
- **Lightweight:** Build agent（單一 fresh agent，載入團隊 build skill，不跑 phase loop）
- Verify agent（載入團隊 verify skill）— Full always，Lightweight 僅 PASS-BUILD-WITH-VERIFY
