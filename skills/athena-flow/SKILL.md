...
掃 `.athena/skills/*/SKILL.md` frontmatter 的 `stage` 欄位建對應表；路由所需 standard stage 缺 skill → 停止＋逐字輸出 `templates/msg-missing-skill.md`；flow-inline 缺 → 用 plugin 預設不停止；同一 stage 被 ≥ 2 個 skill 宣告 → 停止並逐字輸出 `templates/msg-stage-conflict.md`（多流程需求用 index skill 路由）。對應表範例與各路由所需 skill 清單：見 stage-contracts.md「Skill Discovery」。
